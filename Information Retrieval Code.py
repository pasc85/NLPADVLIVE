...
def read_data():
    ...
    url ='https://raw.githubusercontent.com/salihadfid1/NLPADVLIVE/master/IATI.csv'
    raw_data_orig = pd.read_csv(url) 
...
df = pd.read_csv('C:/IR Course/Adv -IR/IATI10k.csv', header = 0, encoding="iso-8859-1")
...
def calc_docscore(pdf, pqry):
    ...
    f_df2  = pd.DataFrame(columns = col_names)
    for index, row in pdf.iterrows(): 
        rank = []
        docscore = 0
        ...
        f_df2.loc[index] = pd.Series({'Description':row['description'], 'score':docscore})
    return f_df2
